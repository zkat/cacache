...
function moveToDestination (tmp, cache, sri, opts, errCheck) {
  ...
  return fixOwner.mkdirfix(
    ...
    const needsMode = opts.mode != null
    const needsTime = opts.mtime != null
    if (needsMode || needsTime) {
      return
      errCheck && errCheck()
      return BB.join(
        needsMode && fs.chmodAsync(destination, opts.mode),
        needsTime && fs.utimesAsync(destination, Date.now(), new Date(opts.mtime))
      )
    }
  })
}
