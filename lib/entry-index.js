...
function insert (cache, key, digest, opts) {
  ...
  const lockfile = bucket + '.lock'
  return fixOwner.mkdirfix(
    path.dirname(bucket), opts.uid, opts.gid
  ).then(() => (
    Promise.using(
      Promise.resolve(slocket(lockfile)).disposer(lock => lock.release()),
      () => (
        fs.statAsync(bucket).catch(err => {
          ...
          return fs.appendFileAsync(bucket, e).then(() => entry)
        })
      )
    )
  )).then(entry => {
